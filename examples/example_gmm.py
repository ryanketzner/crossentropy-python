...
Perform Cross‐Entropy + Importance Sampling with a Gaussian Mixture proposal.
Find the probability that a Gaussian random variable lies in a small circle.
...
def main():
    ...
    parallel = False
    extras = 0
    num_components = 5  # number of mixture components

    # Update function for CE: weighted MLE of a GaussianMixture
    ...
    # Run Cross‐Entropy to find mixture proposal q_dist
    ...
    print(f"GaussianMixture Cross‐Entropy completed in {n_iters} iterations. Success = {completed}")

    # Run Importance Sampling with final mixture proposal q_dist
    ...
    plt.xlabel("x₁")
    plt.ylabel("x₂")
    plt.title("Importance Sampling with Gaussian Mixture Proposal")
    plt.legend()
    plt.tight_layout()
    plt.show()
