...
Perform Cross‐Entropy + Importance Sampling with a single Gaussian proposal.
...
def main():
    # Define target Gaussian distribution p
    ...
    # Update function for CE: weighted MLE of Gaussian
    def update_fn(samples: np.ndarray, weights: np.ndarray) -> Gaussian:
        return Gaussian.weighted_mle(samples, weights)
    ...
    plt.xlabel("x₁")
    plt.ylabel("x₂")
    plt.title("Importance Sampling with Single Gaussian Proposal")
    plt.legend()
    plt.axis("equal")
    plt.tight_layout()
    plt.show()
